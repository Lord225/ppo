...
params.observation_space = (84, 84, 3)
...
params.iters_per_episode = 64
params.mini_batch_size = params.batch_size // params.iters_per_episode 
...
def get_model() -> tf.keras.Model:
    ...
    x = tf.keras.layers.Conv2D(64, 4, strides=2, activation="relu")(inputs)
    x = tf.keras.layers.Conv2D(128, 2, strides=2, activation="relu")(x)
    x = tf.keras.layers.Conv2D(128, 3, strides=1, activation="relu")(x)
    x = tf.keras.layers.Conv2D(128, 2, strides=1, activation="relu")(x)
...
def run():
    ...
    memory = ReplayMemory(15_000, params.observation_space)
